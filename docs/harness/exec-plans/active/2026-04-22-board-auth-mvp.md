...
- 2026-04-29 R2 connection check: board image upload code is present, but local `.env.local` and Vercel production do not yet define `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, or `R2_PUBLIC_BASE_URL`, so live uploads will return the configured `r2_not_configured` path until Cloudflare R2 credentials and a public bucket domain are added. `.env.example` now documents the required keys and `npm run test:board-r2-upload-contract` guards that contract.
- 2026-04-29 live R2 setup and board write UX follow-up: `star-hosaga.com` was registered, `images.star-hosaga.com` was connected to the `star-hosaga-board-images` R2 bucket, Vercel production/preview R2 env vars were added, and a live board image upload rendered from the custom domain. Board list dates now render in `Asia/Seoul` 24-hour time, the 10-character content minimum was removed, image/video inputs were aligned, and the composer preview was reduced to a compact size. Added `npm run test:board-write-contract` assertions for these UX contracts.
- 2026-04-29 board attachment policy follow-up: board posts still allow one image attachment and one video URL, but upload limits now use 10MB for jpg/png/webp and 20MB for GIF. The board image API accepts 22MB multipart bodies to leave overhead for the GIF limit, while `lib/r2.ts` keeps the per-mime validation before upload. `BoardPostComposer` now also supports local image drag-and-drop in the image field, using the same upload path as file selection and paste.
- 2026-04-29 board delete follow-up: board post deletion is limited to a valid admin session or the original SOOP author session. The delete API removes the DB row and then attempts to delete the matching `board/` object from the configured R2 public domain, logging R2 cleanup failures without restoring the deleted row.
- 2026-04-29 board edit follow-up: board post editing is limited to the same valid admin/original SOOP author rule. The edit page and `PATCH /api/board/[id]` only update title, content, image URL, and video URL; author identity, category, download URL, and timestamps remain server-controlled.
